...
#docker build -t ${IMG} .
#docker buildx build --platform linux/amd64 -t jojang91/sample01:latest --push .
#docker buildx build --platform linux/arm64 -t jojang91/sample01:latest --push .
docker buildx build --platform linux/arm64 -t jojang91/sample01:latest .
...
# kubectl set image deploy ${MODULE} ${MODULE}=${IMGdocker rmi $(ku docker images -q)
#mvn clean
echo Module Name:${MODULE}
